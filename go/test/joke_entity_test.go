...
func TestJokeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JOKE_FATHER_TEST_JOKE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func jokeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JOKE_FATHER_TEST_JOKE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JOKE_FATHER_TEST_JOKE_ENTID"])
	if idmapResolved == nil {
		idmapResolved = core.ToMapAny(idmap)
	}

	if env["JOKE_FATHER_TEST_LIVE"] == "TRUE" {
		...
		client = sdk.NewJokeFatherSDK(core.ToMapAny(mergedOpts))
	}
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["JOKE_FATHER_TEST_EXPLAIN"] == "TRUE",
		live:          live,
