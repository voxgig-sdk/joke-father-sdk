...
func TestJokeEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JOKEFATHER_TEST_JOKE_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func jokeBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JOKEFATHER_TEST_JOKE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JOKEFATHER_TEST_JOKE_ENTID"])
	...
	return &entityTestSetup{
		...
		idmap:         idmapResolved,
		env:           env,
		explain:       env["JOKEFATHER_TEST_EXPLAIN"] == "TRUE",
		live:          live,
