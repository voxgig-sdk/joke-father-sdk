...
import (
	...
	omni "github.com/voxgig-sdk/joke-father-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JokeFatherError); is {
				out := map[string]any{
					"name":    "JokeFatherError",
...
func omniCtx(arg any, client *sdk.JokeFatherSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JokeFatherSDK); is {
			client = live
