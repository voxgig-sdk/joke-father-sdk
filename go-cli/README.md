...
boru-driven command-line client **and** interactive REPL for the JokeFather
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/joke-father-cli)
make build

# 2. See usage (words, entities, env vars)
./joke-father-cli --help

# 3. Provide credentials once, via the environment
export JOKE_FATHER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./joke-father-cli load 1 joke            # {id:1} shorthand
./joke-father-cli load '{id:1}' joke       # explicit match map

# 5. Override the API base URL for a single call
JOKE_FATHER_BASE=https://api.example.com ./joke-father-cli load 1 joke

# 6. No arguments -> interactive REPL
./joke-father-cli
joke-father> load 1 joke
joke-father> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/joke-father-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/joke-father-cli load 1 joke
   ```
...
```sh
./joke-father-cli load 1 joke          # scalar shorthand for {id:1}
./joke-father-cli load '{id:1}' joke     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `joke-father>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/joke-father-cli`. |
...
(`./joke-father-cli <expr>`) and interactively (the REPL), and expressions compose the
