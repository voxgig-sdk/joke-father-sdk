// Command JokeFather-cli is a CLI + REPL for the JokeFather SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      JOKE_FATHER_APIKEY (api key), JOKE_FATHER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: JOKE_FATHER_APIKEY carries the API key and
	// JOKE_FATHER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("JOKE_FATHER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("JOKE_FATHER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewJokeFatherSDK(opts)
