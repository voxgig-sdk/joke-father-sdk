...
[MCP](https://modelcontextprotocol.io) server exposing the JokeFather SDK as
two agent tools — `joke-father_list` and `joke-father_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/joke-father-mcp)
make build

# 2. Provide credentials via the environment
export JOKE_FATHER_APIKEY=sk_live_xxx
...
```jsonc
// joke-father_list: first page of records
{ "entity": "joke" }
{ "entity": "joke", "query": { } }

// joke-father_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/joke-father-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user joke-father \
     -- "$PWD"/dist/*/joke-father-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `joke-father_list` and `joke-father_load` tools now appear
   in new sessions. Ask the agent to *"list joke using joke-father"*
   and it calls `joke-father_list` with `{"entity":"joke"}`.
...
```sh
export JOKE_FATHER_APIKEY=sk_live_xxx            # API key
export JOKE_FATHER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `joke-father_list` tool
...
### Call the `joke-father_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `joke-father_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `joke-father_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `JOKE_FATHER_APIKEY` | API key sent with every request. |
| `JOKE_FATHER_BASE` | Optional override of the API base URL. |
