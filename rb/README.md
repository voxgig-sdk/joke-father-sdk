...
The Ruby SDK for the JokeFather API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = JokeFatherSDK.test(testopts, sdkopts)
```
...
raise a `JokeFatherError` (a `StandardError` subclass), so wrap
...
```
rb/
├── JokeFather_sdk.rb       -- Main SDK module
...
The main module (`JokeFather_sdk`) exports the SDK class
