...
require_relative 'JokeFather_types'
...
class JokeFatherSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = JokeFatherHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = JokeFatherHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, JokeFatherFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    JokeFatherUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = JokeFatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = JokeFatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = JokeFatherHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => JokeFatherError.new(
        "#{op}_allow",
        "JokeFatherSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = JokeFatherHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = JokeFatherError.new(
        "graphql_error", "JokeFatherSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
