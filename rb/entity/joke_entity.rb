...
class JokeEntity
  ...
  def data_set(args)
    if args
      @_data = JokeFatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = JokeFatherHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Joke, Hash] the loaded Joke; raises JokeFatherError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JokeFatherHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
