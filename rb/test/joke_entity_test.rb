...
class JokeEntityTest < Minitest::Test
  def test_create_instance
    testsdk = JokeFatherSDK.test(nil, nil)
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set JOKE_FATHER_TEST_JOKE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def joke_basic_setup(extra)
  ...
  entid_env_raw = ENV["JOKE_FATHER_TEST_JOKE_ENTID"]
  ...
  idmap_resolved = Helpers.to_map(
    env["JOKE_FATHER_TEST_JOKE_ENTID"])
  ...
  if env["JOKE_FATHER_TEST_LIVE"] == "TRUE"
    ...
    client = JokeFatherSDK.new(Helpers.to_map(merged_opts))
  end
