...
// JokeFather SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = JokeFatherOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(JokeFatherOmni::structfix($now, Struct::undef()))
                        !== json_encode(JokeFatherOmni::structfix($val, Struct::undef()))) {
