...
{
    private static function utility(): JokeFatherUtility
    {
        return new JokeFatherUtility();
    }
    ...
    private static function ctx(array $over = []): JokeFatherContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new JokeFatherContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new JokeFatherOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof JokeFatherError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new JokeFatherOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = JokeFatherMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new JokeFatherOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = JokeFatherMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new JokeFatherOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = JokeFatherMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JokeFatherSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = JokeFatherMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new JokeFatherResponse([]),
            'result' => new JokeFatherResult([]),
        ]);
        [, $err] = JokeFatherMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new JokeFatherSpec([]),
            'response' => null,
            'result' => new JokeFatherResult([]),
        ]);
        [, $err] = JokeFatherMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new JokeFatherSpec(['step' => 's']),
            'response' => new JokeFatherResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new JokeFatherResult([]),
        ]);
        [, $err] = JokeFatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new JokeFatherSpec(['step' => 's']),
            'response' => new JokeFatherResponse(self::resp(200, ['v' => 1])),
            'result' => new JokeFatherResult([]),
        ]);
        [, $err] = JokeFatherMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new JokeFatherSpec(['step' => 's']),
            'response' => new JokeFatherResponse(self::resp(200, ['v' => 2])),
            'result' => new JokeFatherResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        JokeFatherMakeResponse::call($ctx);
    ...
    {
        $preset = new JokeFatherResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new JokeFatherSpec([]),
            'response' => new JokeFatherResponse([]),
            'result' => new JokeFatherResult([]),
        ...
        [$got, $err] = JokeFatherMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new JokeFatherResult([])]);
        [, $err] = JokeFatherMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new JokeFatherSpec([]), 'result' => null]);
        [, $err] = JokeFatherMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JokeFatherSpec(['step' => 's']),
            'result' => new JokeFatherResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new JokeFatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JokeFatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new JokeFatherSpec(['step' => 's']),
            'result' => new JokeFatherResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new JokeFatherOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = JokeFatherMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new JokeFatherResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new JokeFatherSpec([]),
            'result' => new JokeFatherResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = JokeFatherMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JokeFatherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new JokeFatherError('boom', 'boom');
        $utility->fetcher = function (JokeFatherContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JokeFatherSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = JokeFatherMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (JokeFatherContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = JokeFatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(JokeFatherResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        JokeFatherMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (JokeFatherContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new JokeFatherSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = JokeFatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new JokeFatherResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new JokeFatherSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = JokeFatherMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = JokeFatherMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = JokeFatherMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new JokeFatherResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, JokeFatherDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new JokeFatherResult(['ok' => false])]);
        $this->expectException(JokeFatherError::class);
        JokeFatherDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new JokeFatherResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', JokeFatherMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        JokeFatherMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new JokeFatherBaseFeature();
        $b = new JokeFatherBaseFeature();
        JokeFatherFeatureAdd::call($ctx, $a);
        JokeFatherFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): JokeFatherBaseFeature
    {
        $f = new JokeFatherBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        JokeFatherFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): JokeFatherContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new JokeFatherSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = JokeFatherPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        JokeFatherPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new JokeFatherResponse(['headers' => null]),
            'result' => new JokeFatherResult([]),
        ]);
        JokeFatherResultHeaders::call($ctx);
