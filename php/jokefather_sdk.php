...
class JokeFatherSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = JokeFatherHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = JokeFatherHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!JokeFatherFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, JokeFatherFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return JokeFatherUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = JokeFatherHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = JokeFatherHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = JokeFatherHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new JokeFatherError($op . "_allow",
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = JokeFatherHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new JokeFatherError("graphql_error",
                "JokeFatherSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
