...
module JokeFatherUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = JokeFatherResponse.new({})
    ...
    if fetch_err
      ...
      response = JokeFatherResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JokeFatherResponse.new(fetched)
    else
