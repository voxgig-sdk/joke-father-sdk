...
class FtCtrl extends JokeFatherControl
{
...
class FtClient
{
    ...
    private JokeFatherUtility $utility;

    public function __construct(JokeFatherUtility $utility, array $options)
    {
    ...
    public function get_utility(): JokeFatherUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JokeFatherContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JokeFatherContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JokeFatherContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JokeFatherConfig::make_feature($name);
    ...
    private static function buildUrl(JokeFatherSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JokeFatherError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JokeFatherResponse($fetched);
        }
    ...
    private function fail(JokeFatherContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JokeFatherContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
