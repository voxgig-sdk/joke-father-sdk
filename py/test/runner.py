# JokeFather SDK test runner

from __future__ import annotations
import os
import json

from jokefather_sdk.utility.voxgig_struct import voxgig_struct as vs


class JokeFatherTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JokeFatherTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JokeFatherTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JokeFatherTestRunner.getenv("JOKE_FATHER_TEST_LIVE")
        override = JokeFatherTestRunner.getenv("JOKE_FATHER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JokeFatherTestRunner.getenv(key)
        ...
        explain = JokeFatherTestRunner.getenv("JOKE_FATHER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["JOKE_FATHER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if JokeFatherTestRunner._test_control is not None:
            return JokeFatherTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JokeFatherTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JokeFatherTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JokeFatherTestRunner.load_test_control()
...
def load_env_local():
    JokeFatherTestRunner.load_env_local()


def env_override(m):
    return JokeFatherTestRunner.env_override(m)


def entity_data(v):
    return JokeFatherTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JokeFatherTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JokeFatherTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JokeFatherTestRunner.load_test_control()


def live_client_options():
    return JokeFatherTestRunner.live_client_options()


def live_delay_ms():
    return JokeFatherTestRunner.live_delay_ms()
