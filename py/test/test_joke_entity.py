    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set JOKEFATHER_TEST_JOKE_ENTID JSON to run live")
        client = setup["client"]
...
def _joke_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("JOKEFATHER_TEST_JOKE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("JOKEFATHER_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
                "apikey": env.get("JOKEFATHER_APIKEY"),
            },
            extra or {},
        ])
        client = JokeFatherSDK(helpers.to_map(merged_opts))

    _live = env.get("JOKEFATHER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("JOKEFATHER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
