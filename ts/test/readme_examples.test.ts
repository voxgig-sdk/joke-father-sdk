...
//     (\`new JokeFatherSDK(...)\` / \`JokeFatherSDK.test(...)\` become
//     \`JokeFatherSDK.test({ entity: ... })\`), transpiled to JS, and run
...
// rewrite real client construction (\`new JokeFatherSDK(...)\` and any
// \`JokeFatherSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JokeFatherSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JokeFatherSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
