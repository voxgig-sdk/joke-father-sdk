...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.JOKEFATHER_TEST_LIVE ||
    'TRUE' === process.env.JOKEFATHER_TEST_OVERRIDE
  ) {
  ...
  m.JOKEFATHER_TEST_EXPLAIN = process.env.JOKEFATHER_TEST_EXPLAIN || m.JOKEFATHER_TEST_EXPLAIN
